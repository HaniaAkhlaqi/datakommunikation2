...
# Set different TCP versions for nodes
...
#                   ns.core.TypeIdValue(ns.core.TypeId.LookupByName("ns3::TcpReno")))
...
#######################################################################################
# CREATE UDP CLIENT AND SERVER (between 1 and  3)
#
def SetupUdpConnection(srcNode, dstNode, dstAddr, startTime, stopTime):
  ...
  on_off_tcp_helper = ns.applications.OnOffHelper("ns3::UdpSocketFactory",
                          ns.network.Address(ns.network.InetSocketAddress(dstAddr, 8080)))
  on_off_tcp_helper.SetAttribute("DataRate",
                      ns.network.DataRateValue(ns.network.DataRate(int(cmd.on_off_rate))))
  on_off_tcp_helper.SetAttribute("PacketSize", ns.core.UintegerValue(1500))
...
#Enable logging for TCP congestion control
#ns.core.LogComponentEnable("TcpNewReno", ns.core.LOG_LEVEL_INFO)
